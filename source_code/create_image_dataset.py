# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 11:28:54 2022
...
def ImageFolderCSV(source_file_path, destination_path):
    files_in_train = sorted(os.listdir(source_file_path))
    
    images=[i for i in files_in_train]


    df = pd.DataFrame()
    df['images']=[source_file_path+str(x) for x in images]

    df.to_csv(destination_path, header=None)
